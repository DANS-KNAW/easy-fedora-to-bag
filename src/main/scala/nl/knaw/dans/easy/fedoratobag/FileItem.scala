...
object FileItem {
  ...
  def apply(fileInfo: FileInfo, isOriginalVersioned: Boolean): Try[Node] = Try {
    ...
    val originalPath = if (fileInfo.originalPath != fileInfo.path) <afm:keyvaluepair><afm:key>ORIGINAL_FILE_PATH</afm:key><afm:value>{ fileInfo.originalPath }</afm:value></afm:keyvaluepair>
                       else Text("")
